  ...
  return (
    <div className="flex items-center gap-2">
       ...
       >
         ...
         <span className="whitespace-nowrap">{user?.primaryEmailAddress?.emailAddress ?? copy.signedInAs}</span>
       </span>
       <Link href="/authors-world?focus=me" className="neon-control inline-flex max-w-40 items-center gap-1.5 truncate px-3 py-2 font-mono text-[10px] uppercase tracking-[0.12em] text-[#b9f7ff]" title={user?.primaryEmailAddress?.emailAddress ? `${copy.signedInAs}: ${user.primaryEmailAddress.emailAddress}` : copy.myPortal}>
          ...
          <div className="site-header-inner flex w-full min-w-0 flex-col items-center justify-between gap-2 px-3 py-2.5 sm:px-5 sm:py-3 lg:px-7 2xl:flex-row 2xl:gap-5">
            ...
            <div className="site-header-actions flex w-full min-w-0 flex-nowrap items-center justify-between gap-1.5 overflow-x-auto whitespace-nowrap pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-3 2xl:w-auto 2xl:justify-end 2xl:overflow-visible 2xl:pb-0">
              ...
              <button
                ...
                className="site-header-cart apoc-card relative inline-flex shrink-0 items-center gap-1 px-2 py-1.5 text-white/70 transition-colors hover:text-[#00f0ff] sm:gap-2 sm:px-3 sm:py-2"
                style={{ borderRadius: "12px" }}
              >
