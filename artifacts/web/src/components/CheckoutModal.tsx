...
interface Country {
  ...
  flag: string;
  name: string;
  currency: "UAH" | "USD" | "CAD" | "PLN";
  rate: number;
}

const COUNTRIES: Country[] = [
  { code: "UA", flag: "🇺🇦", name: "Україна", currency: "UAH", rate: 1 },
  { code: "US", flag: "🇺🇸", name: "США", currency: "USD", rate: 1 / 41 },
  { code: "CA", flag: "🇨🇦", name: "Канада", currency: "CAD", rate: 1 / 30 },
  { code: "PL", flag: "🇵🇱", name: "Польща", currency: "PLN", rate: 1 / 10 },
];

const CURRENCY_SYMBOL: Record<Country["currency"], string> = {
  UAH: "₴",
  USD: "$",
  CAD: "CA$",
  PLN: "zł",
...
export function CheckoutModal({ open, onClose }: Props) {
  ...
  const convertedTotal = useMemo(() => {
    const value = total * country.rate;
    return country.currency === "UAH" ? Math.round(value) : Math.round(value * 100) / 100;
  }, [total, country]);
                ...
                {items.length === 0 ? (
                  ...
                  items.map((line) => (
                    <li key={line.key} className="flex items-center justify-between gap-3 font-mono text-sm">
                      ...
                      <div className="text-[#00f0ff]">{line.price * line.qty} ₴</div>
                    </li>
                  ))
                )}
              </ul>
              <div className="mt-3 flex items-center justify-between font-mono">
                ...
                <div className="text-right">
                  <div className="text-2xl text-[#00f0ff]">
                    {convertedTotal} {CURRENCY_SYMBOL[country.currency]}
                  </div>
                  ...
                  {COUNTRIES.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="uppercase tracking-[0.2em]">{c.currency}</span>
                      </button>
                    );
                  })}
                </div>
